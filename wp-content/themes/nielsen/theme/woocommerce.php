...
if ( !function_exists( 'yit_shop_rating' ) ) {
    ...
    function yit_shop_rating() {
        ...
        echo '<div class="woocommerce-product-rating">'.$product->get_rating_html().'</div>';
    }
}
...
if ( !function_exists( 'yit_get_current_cart_info' ) ) {
    ...
    function yit_get_current_cart_info() {
        ...
        return array(
            sprintf('%s %s', $items, _n('item','items',$items,'yit') ),
            $total
        );
    }
}
...
function yit_woocommerce_account_menu_items( $menu_list ) {
    ...
    if( yit_get_option( 'my-account-download' ) == 'no' ) {
        unset( $menu_list[ 'downloads' ] );
    }
...
function yit_get_myaccount_menu_icon( $endpoint ) {
    ...
    $icon_list = apply_filters( 'yit_get_myaccount_menu_icon_list', array(
            'dashboard'          => '&#xe3e6;',
            'orders'             => '&#xe443;',
            'downloads'          => '&#xe3c7;',
            'edit-address'       => '&#xe3f2;',
...
if ( ! function_exists( 'yit_wc_product_post_class' ) ) {
    ...
    function yit_wc_product_post_class( $classes, $class = '', $post_id = '' ) {
        ...
        if ( $product ) {
            ...
            if ( ( 0 === ( $woocommerce_loop['loop'] - 1 ) % $woocommerce_loop['columns'] || 1 === $woocommerce_loop['columns'] ) && $woocommerce_loop['loop'] > 1 ) {
                //$classes[] = 'first';
            }
            if ( ( 0 === $woocommerce_loop['loop'] % $woocommerce_loop['columns'] ) && $woocommerce_loop['loop'] > 1 ) {
                $classes[] = 'last';
            }
...
function yit_wc_2_6_removed_unused_template () {
    ...
    if( function_exists( 'yit_remove_unused_template' ) ) {
        ...
        $files = array( 'checkout/form-shipping.php'  , 'myaccount/form-login.php' , 'myaccount/my-account.php' , 'myaccount/my-account-menu.php' , 'myaccount/my-orders.php' , 'single-product/review.php' );
