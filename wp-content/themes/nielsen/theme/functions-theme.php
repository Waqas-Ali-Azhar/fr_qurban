    ...
    if( !wp_get_nav_menu_object( $menuname ) ) {
        if( is_shop_installed() ) {
            ...
            if( $my_account_id ) {
                ...
                $order_end_point = version_compare( preg_replace( '/-beta-([0-9]+)/', '', WC()->version ), '2.6', '>=' ) ? 'orders' : 'view-order';

                wp_update_nav_menu_item($menu_id, 0, array(
                    'menu-item-title' =>  __('My Orders', 'yit'),
                    'menu-item-classes' => $order_end_point,
                    'menu-item-url' => wc_get_endpoint_url( $order_end_point , '',  $my_account_url ),
                    'menu-item-status' => 'publish'));
